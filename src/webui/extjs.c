...
extjs_dvr(http_connection_t *hc, const char *remain, void *opaque)
{
  ...
  if(!strcmp(op, "recordEvent") || !strcmp(op, "recordSeries")) {
    ...
    htsmsg_add_str(r, "filename_advanced_filename", cfg->dvr_filenaming_advanced.filename_format ? cfg->dvr_filenaming_advanced.filename_format : "");
    ...
    htsmsg_add_msg(r, "filename_advanced_regex", regexes);
#endif
   ...
   if((s = http_arg_get(&hc->hc_req_args, "postExtraTime")) != NULL)
    ...
    if((s = http_arg_get(&hc->hc_req_args, "filename_mode")) != NULL)
      if (dvr_filenaming_set_mode(cfg, atoi(s)))
        changed = 1;
    if((s = http_arg_get(&hc->hc_req_args, "filename_advanced_filename")) != NULL)
